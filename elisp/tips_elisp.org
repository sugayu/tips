#+title: *Elisp---Emacs LISP*
#+AUTHOR: sugayu

* マニュアル
- [[info:elisp][Emacs Lisp Manual]]

* 基本構文

** 条件分岐
- [[info:elisp#Conditionals][elisp#Conditionals]]

単純な条件分岐には ~if~ が使える。
#+begin_src emacs-lisp :results raw drawer :eval never-export
  (if t
      (message "Print this if True")
    (message "Print this if false"))
#+end_src

#+RESULTS:
:results:
Print this if True
:end:

~t~ のときのみの実行は ~when~ が使える。
#+begin_src emacs-lisp :results raw drawer :eval never-export
  (when t
    (message "Print this when True.")
    (message "Recieve sequential commands."))
#+end_src

#+RESULTS:
:results:
Recieve sequential commands.
:end:


複数の条件分岐には ~cond~ が使える。
#+begin_src emacs-lisp :results raw drawer :eval never-export
  (cond ((eq "a" "not a") "False")
        (t "True"))
#+end_src

#+RESULTS:
:results:
True
:end:

パターンマッチには ~pcase~ が使える。
#+begin_src emacs-lisp :results raw drawer :eval never-export
  (pcase "answer"
    (`answer 'ans)
    ((pred stringp) 'string)
    (_ 'fallback))
#+end_src

#+RESULTS:
:results:
string
:end:

- [[https://qiita.com/t-suwa/items/20a4ebf37b0a57ff88b2][Emacs：パターンマッチでスクレイピング #Emacs - Qiita]]

** ループ
Lispでは基本的にループは使わずに、 ~map~ 系の関数を使うはず。それでも一応ループも用意されている。

リストに対して同じ操作を繰り返すときには ~dolist~ が使える。
#+begin_src emacs-lisp :results raw drawer :eval never-export
  (dolist (buffer (buffer-list))
    (kill-buffer (buffer-name buffer)))
#+end_src

** 判定
- derived-mode-p :: 現在のbufferのmode判定 ~(derived-mode-p 'org-mode)~ 。返り値はmode名。
- file-exists-p :: 文字列のファイル名が存在するか。

* ファイルとバッファ
- [[info:elisp#Visiting Files][elisp#Visiting Files]]
- [[info:elisp#Buffer File Name][elisp#Buffer File Name]]
ファイルに対して、バッファは一時的な編集の場であり、
一般に言う「ファイルを開く」はEmacsではファイル内容をバッファに読み込む、と言う。

** 便利な変数・関数
ファイル名とバッファ名は異なるので、与える引数の間違いに注意する。
- v ~buffer-file-name~ :: 現在のバッファの名前
- f ~buffer-name~ :: バッファ自体は特殊な名前構造を持っている。バッファの名前を文字列で返す。
- f ~file-name-directory~ :: 与えたファイルが存在するディレクトリを返す。

** 作業するカレントバッファを設定する
非対話的に、一時的に別のバッファを操作する場合、 ~set-buffer~ でそのバッファを設定する。
#+begin_src emacs-lisp :results raw drawer :eval never-export
  (save-current-buffer
    (set-buffer (get-buffer-create "new-buffer")))
#+end_src

- [[info:elisp#Current Buffer][elisp#Current Buffer]]

** バッファをウィンドウに表示する
選択したバッファをウィンドウに表示して対話的に操作したいときは ~pop-to-buffer-same-window~ を使う。
#+begin_src emacs-lisp :results raw drawer :eval never-export
  (let* ((path (expand-file-name ".emacs.d/init.el" (getenv "HOME")))
         (buffer (or (get-file-buffer path)
                     (find-buffer-visiting path))))
    (when buffer
      (pop-to-buffer-same-window buffer)))
#+end_src

異なるウィンドウにバッファを表示するには ~pop-to-buffer~ を使う。

~switch-to-buffer~ もあるが、こちらを使えばいいらしい。知らんけど。
- [[https://emacs.stackexchange.com/questions/27712/switch-to-buffer-vs-pop-to-buffer-same-window][`switch-to-buffer` vs. `pop-to-buffer-same-window` - Emacs Stack Exchange]]
- [[info:elisp#Switching Buffers][elisp#Switching Buffers]]

** ファイルを開いているバッファを得る
以下の二つの関数を使うと目的のバッファが見つかると考えられる。
- f ~get-file-buffer~ :: ファイルをvisitしているバッファを返す
- f ~find-buffer-visiting~ :: ファイルをvisitしているかもしれないバッファを返す
バッファが見つからない場合、これらは ~nil~ を返す。

例はorg-macs.elより:
#+begin_src emacs-lisp :results raw drawer :eval never-export
  (defun org-find-base-buffer-visiting (file)
    "Like `find-buffer-visiting' but always return the base buffer.
  FILE is the file name passed to `find-buffer-visiting'."
    (let ((buf (or (get-file-buffer file)
                   (find-buffer-visiting file))))
      (org-base-buffer buf)))
#+end_src

** 新しいファイルを開く
- f ~find-file-noselect~ :: ファイルをバッファに読み込み、バッファを返す。それを現在のバッファにしたり表示したりはしない。

** ファイルに書き込む
新規や既存のファイルを開いて書き込むだけなら、 ~with-temp-file~ を使う。
#+begin_src emacs-lisp :results raw drawer :eval never-export
  (with-temp-file "filename"
    (insert "#+title: This is title\n"))
#+end_src

~find-file~ や ~find-file-noselect~ はファイルを開いたあとに、
メジャーモードの指定やhookの実行など非対話的モードでは必要ない操作や副作用が多い。
- [[https://emacs.stackexchange.com/questions/2868/whats-wrong-with-find-file-noselect][What's wrong with `find-file-noselect`? - Emacs Stack Exchange]]

** ファイルをリストする
~directory-files~ で指定したディレクトリのファイルをリストにすることができる。
オプションは
- FULL :: 絶対パスを返す。
- MATCH :: 正規表現マッチを行う。
- NOSORT :: 並びかえない。あとから自分で並びかえるときに使う。
- COUNT :: 指定した数だけしかファイルを返さない。
例として、実際に作った、現在のディレクトリ上にある
特定の名前を持つファイルリストを作って逆順に並びかえるコマンドを挙げる。
#+begin_src emacs-lisp :results raw drawer :eval never-export
  (defun my/directory-journal-files ()
    (reverse
     (sort
      (directory-files (file-name-directory buffer-file-name) nil "20..w...org" t)
      'string-lessp)))
#+end_src

* カーソル移動とマーカー

** バッファ内でカーソルを移動させる
~goto-char~ を使う。
引数には ~POSITION~ を与える。1がバッファの先頭になる。

** バッファの開始点と終了点に移動する
- f ~point-min~ :: バッファ内で許されうる最小の位置を返す。
- f ~point-max~ :: バッファ内で許されうる最大の位置を返す。
おそらく「許されうる」がポイントで、
バッファがnarrowingなどで狭まっていた場合には、狭まった範囲の最小値・最大値を返すと思われる。

以下の式を評価する(~C-x C-e~)とバッファの最後に移動する。
#+begin_src emacs-lisp :results raw drawer :eval never-export
  (goto-char (point-max))
#+end_src

* リスト処理
** リスト基本
- f ~length~ :: リストの長さを返す。
- f ~member~ :: リストが要素を含むかどうか調べる。

** リスト結合
#+begin_src emacs-lisp :results raw drawer :eval never-export
  (append '(1 2 3) '(3 4 5))
#+end_src

#+RESULTS:
:results:
| 1 | 2 | 3 | 3 | 4 | 5 |
:end:

** リストの要素処理
要素の削除は ~delete~ (破壊的) か ~remove~ (非破壊的)。
#+begin_src emacs-lisp :results raw drawer :eval never-export
  (let* ((a '(2 3 4))
         (b '(2 3 4))
         (c (remove 3 b)))
    (delete 3 a)
    (append a '("/") b '("/") c))
#+end_src

#+RESULTS:
:results:
| 2 | 4 | / | 2 | 3 | 4 | / | 2 | 4 |
:end:

** 同一要素に対する処理
重複の無いリストにするには ~delete-dups~ (破壊的) か ~seq-uniq~ (非破壊的) を使う。
#+begin_src emacs-lisp :results raw drawer :eval never-export
  (let ((a '(2 3 4 6 6 3 7 2 4))
        (b '(2 3 4 6 6 3 7 2 4)))
    (delete-dups a)
    (seq-uniq b)
    (append a '("/") b))
#+end_src

#+RESULTS:
:results:
| 2 | 3 | 4 | 6 | 7 | / | 2 | 3 | 4 | 6 | 6 | 3 | 7 | 2 | 4 |
:end:

重複している要素をリストから除くには ~seq-difference~ が使える。
引数の順番は、前者のリストから後者に含まれるものを除く、を意味するので注意する。
#+begin_src emacs-lisp :results raw drawer :eval never-export
  (let ((a '(2 3 4 6 6 3 7 2 4))
        (b '(2 3 4 6)))
    (seq-difference a b))
#+end_src

#+RESULTS:
:results:
| 7 |
:end:

- 便利な関数は [[file:/usr/local/Cellar/emacs-mac@29/emacs-29.4-mac-10.1/Emacs.app/Contents/Resources/lisp/emacs-lisp/seq.el.gz][seq.el.gz]] に含まれていると思われる。

* 文字列処理
** 検索
文字列内の検索には ~string-match~ が正規表現とともに使える。
#+begin_src emacs-lisp :results raw drawer :eval never-export
  (remove nil
          (mapcar (lambda (buffer)
                    (when (string-match (format-time-string "%Yw...org") (buffer-name buffer))
                      (buffer-name buffer)))
                  (buffer-list)))
#+end_src

#+RESULTS:
:results:
| 2025w40.org | 2025w39.org |
:end:

* 変数の設定方法
** defvar
Lisp に標準的に備わっている関数。変数をグローバルに定義する。
既に変数に値が設定されている場合、何もしない。また、定義と同時に値を格納しなくてもよい。
変数は動的なスコープを持っているので、値が変更されると既に定義されている他の関数に影響を及ぼす。
docstringsを付与することができ、ここに変数の説明を書くことでヘルプから参照することができる。

#+begin_quote
You will be one step closer to achieving Emacs zen[1].
In other words, it is more idiomatic and elegant to define variables prior to their use.
#+end_quote
[[https://emacs.stackexchange.com/questions/38284/init-el-setq-or-defvar][variables - init.el - setq or defvar - Emacs Stack Exchange]]

** setq
Lisp に標準的に備わっている関数。変数に値を格納する。
変数が事前に定義されていない場合の振舞いは本来的には決まっていない。
Emacs ではおそらく新たに変数が定義されることになると思われる。
望むらくは、事前に ~defvar~ もしくは ~let~ などで事前に変数が定義されている必要がある。
変数に値を設定するのが仕事なので、 変数がlocal-variable の場合はlocalな値を設定する。

** defvar-local
ローカル変数を定義する。同時に格納される値はグローバル (デフォルト) となる。
~setq~ で値を格納すると、バッファローカル変数の値が変更される。

** setq-local
バッファローカル変数に値を設定する。
これはグローバル変数に対してもローカルな値を格納することができ、
この場合は ~make-local-variable~ の後に ~setq~ をすることと振舞いは同じである。

** setq-default
変数が local-variable かどうかに関わらず、変数をglobalに設定する。
おそらく、 ~defvar-local~ で定義された変数のデフォルト値を変更するためのもの。

** let
そのスコープ内でのみ有効な、一時的な変数を定義する。
内部で変数の値を変更する際には ~setq~ を使い、その値は ~let~ のスコープ外には適用されない。

** customize-set-variable
パッケージの設定変数を変更する。これを使うことで振舞いや初期化に関する変数を変更したことを明示する。
パッケージにおける変数定義の際、 ~:custom-set~ などの性質が定義されていると
~setq~ ではこのプロパティ(関数)が動かないので変数が正しく定義できない可能性がある。
~defcustom~ によって定義された変数については、これを使うのが無難という意見が多い。
この関数は与えられた第2引数を評価してから代入して返すことに注意する。

~use-package~ を使っている場合は ~:custom~ を使うことでこの関数が呼び出されていたが、
現在は ~(load custom-file)~ との変数の二重設定を防ぐために ~custom-theme-set-variables~ に変更されている。

[[help:customize-set-variable]]

** custom-set-variables
~customize~ や ~customize-option~ でインタラクティブに変更された変数は ~custom-file~ に一覧で格納される。
この一覧を読み直すのが ~custom-set-variables~ 。
ユーザーが手で変更することは推奨されていないし、 init.el で使うことも推奨されていないと思われる。
内部で ~user~ を theme とした ~custom-theme-set-variables~ を呼び出している。
この関数は与えられた第2引数を評価せずに表現としてそのまま ~:saved-value~ プロパティに保存し、
変数に値が既に与えられていた場合もしくはパッケージがロードされて変数が定義された場合に
保存していた表現を評価して代入する。
この点で ~customize-set-variable~ とは振舞いが異なるので注意する。

~use-package~ では元となる関数 ~custom-theme-set-variables~ が使われているが、
~customize-option~ で変数の情報を確認したときには設定した内容は正しく反映されていない。
おそらく、 ~use-package~ の関数評価順番の問題で、関数が定義される前に値が格納されるので、
後に変数が定義された際に ~:saved-value~ に格納済みの表現を評価されるが、このときに
元々 ~custom-theme-set-variables~ を通じて定義されている事実や設定したコメントの反映が
うまくなされていないと予想する。
- [[https://macowners.club/posts/setq-vs-customize-set-variable/][Setq vs customize-set-variable | macOS & (open-source) Software]]

[[help:custom-set-variables]]

** setopt
Emacs 29 から導入された。
~customize-set-variable~ の糖衣構文と考えるのが良いらしい。
[[https://www.reddit.com/r/emacs/comments/178em7u/why_was_setopt_introduced_in_emacs_29/][Why was setopt introduced in emacs 29 ? | reddit]]
ただし、 ~setq~ のように複数の変数と値の組を変更可能で、
~custom-file~ には変更内容が記録されない。



* コマンド
** prefix arguments
コマンドの直前に数字を付与することで、コマンドの挙動を変更することができる。
このときの数字を prefix arguments と呼ぶ。

prefix argumentsを付与した挙動をキーバインドに登録したいときは、
prefix argumentsを付与した場合の関数を別で作ってそちらをキーに登録する。
#+begin_src emacs-lisp :results raw drawer :eval never-export
  (defun command-with-prefix ()
    (interactive)
    (let ((current-prefix-arg '(4))) ; C-u
      (call-interactively 'command)))
  (bind-key "C-?" command-with-prefix)
#+end_src

- [[info:elisp#Prefix Command Arguments][elisp#Prefix Command Arguments]]

* 雑多

** 時間
現在の時間を取得するには ~current-time~ を使う。
~time-add~ や ~time-subtract~ で時間の計算ができる。
このとき秒数はかけ算 ~*~ で計算すると分かりやすい。
~format-time-string~ で文字列に変換できる。

#+begin_src emacs-lisp :results raw drawer :eval never-export
  (let* ((now (current-time))
         (next-week (time-add now (* 7 24 3600)))
         (last-week (time-subtract now (* 7 24 3600))))
    (format-time-string "%Y/%m/%d" last-week))
#+end_src

#+RESULTS:
:results:
2025/11/02
:end:

- [[info:elisp#Time Parsing][elisp#Time Parsing]]
- [[info:elisp#Time Calculations][elisp#Time Calculations]]

* Link
- [[info:elisp][elisp]]
